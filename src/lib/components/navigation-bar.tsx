...
export default function NavigationBar() {
	...
	return (
		<div className="flex flex-row items-center h-14 m-4 mt-3 rounded-lg p-4 pr-2 bg-gradient-to-r from-custom-light-blue to-custom-green">
			<Breadcrumb path={path} previousRoutes={previousRoutes} route={route} parallelRoutes={parallelRoutes}/>
...
function Breadcrumb(
	{ path, previousRoutes, route, parallelRoutes }: { path: string[], previousRoutes: Route[], route: Route, parallelRoutes: Route[] },
) {
	return (
		<Ui.Breadcrumb>
			<Ui.BreadcrumbList>
				...
				<ActiveBreadcrumbItem path={path} route={route} parallelRoutes={parallelRoutes}/>
			</Ui.BreadcrumbList>
		</Ui.Breadcrumb>
	);
}
...
function ActiveBreadcrumbItem(
	{ path, route, parallelRoutes }: { path: string[], route: Route, parallelRoutes: Route[] },
) {
